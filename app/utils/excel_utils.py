...
def generate_assets_excel(assets, generated_by_name):
    """
    Generates a professional, enterprise-grade Excel workbook for Asset Inventory.
    Includes filtering out internal IDs and audit metadata.
    """
    wb = Workbook()
    ...
    # Add Metadata Title Block
    ws.cell(row=1, column=1, value="HR Management System - Asset Inventory Report").font = title_font
    
    # Metadata Fields
    metadata = [
        ("Company Name:", "HRMS Enterprise"),
        ("Generated By:", get_clean_name(generated_by_name)),
        ("Generated Date & Time:", datetime.now().strftime("%Y-%m-%d %H:%M:%S")),
        ("Total Records Exported:", len(assets))
    ]
...
def generate_rental_report_excel(matrix_rows, generated_by_name):
    """
    Generates a professional Excel workbook for Rental Management report.
    Columns: Asset Name, Vendor, Rental Start, Rental End, Monthly Rate,
             Apr, May, Jun, Jul, Aug, Sep, Oct, Nov, Dec, Jan, Feb, Mar, Current Status
    """
    ...
    header_fill = PatternFill(start_color="2C3E50", end_color="2C3E50", fill_type="solid")
    zebra_even = PatternFill(start_color="F8F9FA", end_color="F8F9FA", fill_type="solid")
    zebra_odd = PatternFill(start_color="FFFFFF", end_color="FFFFFF", fill_type="solid")

    # Status cell fills
    status_fills = {
        'Paid': PatternFill(start_color="D1FAE5", end_color="D1FAE5", fill_type="solid"),
        'Due This Month': PatternFill(start_color="FEF3C7", end_color="FEF3C7", fill_type="solid"),
        'Upcoming': PatternFill(start_color="DBEAFE", end_color="DBEAFE", fill_type="solid"),
        'Not Applicable': PatternFill(start_color="F3F4F6", end_color="F3F4F6", fill_type="solid"),
    }
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Rental Management"
    ws.views.sheetView[0].showGridLines = True

    # Title
    ws.cell(row=1, column=1, value="HR Management System - Rental Management Report").font = title_font

    # Metadata
    metadata = [
        ("Generated By:", get_clean_name(generated_by_name)),
        ("Generated Date:", datetime.now().strftime("%Y-%m-%d %H:%M:%S")),
        ("Total Records:", len(matrix_rows)),
    ]
    for i, (label, val) in enumerate(metadata, 3):
        ws.cell(row=i, column=1, value=label).font = metadata_label_font
        ws.cell(row=i, column=2, value=val).font = metadata_value_font

    # Headers
    base_headers = ["Asset Name", "Vendor", "Category", "Rental Start", "Rental End", "Renewal Date", "Monthly Rate (₹)"]
    headers = base_headers + MONTH_ORDER + ["Current Status"]
    ...
    # Data rows
    for row_idx, asset in enumerate(matrix_rows, header_row + 1):
        fill = zebra_even if row_idx % 2 == 0 else zebra_odd
        months = asset.get('months', {})

        row_data = [
            f"{asset.get('brand', '')} {asset.get('model', '')}".strip(),
            asset.get('vendor_name', ''),
            asset.get('device_type', ''),
            asset.get('rental_start_date', ''),
            asset.get('rental_end_date', ''),
            asset.get('renewal_date', ''),
            float(asset.get('rental_cost') or 0),
        ]
        ...
        for m_idx, month_name in enumerate(MONTH_ORDER):
            col = len(base_headers) + m_idx + 1
            m_data = months.get(month_name, {})
            status = m_data.get('status', 'Not Applicable')
            amount = m_data.get('amount', 0)
            cell_val = f"₹{amount:,.0f}" if status != 'Not Applicable' else '–'
            cell = ws.cell(row=row_idx, column=col, value=cell_val)
            cell.font = data_font
            cell.fill = status_fills.get(status, fill)
            cell.alignment = center
            cell.border = border

        # Current status
        status_col = len(headers)
        status_val = asset.get('current_status_label', '')
        cell = ws.cell(row=row_idx, column=status_col, value=status_val)
        cell.font = data_font
        cell.fill = status_fills.get(status_val, fill)
        cell.alignment = center
        cell.border = border
