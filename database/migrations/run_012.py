"""
Migration 012 Runner: Announcement Management System
Executes 012_announcement_management.sql against the configured database.

Usage:
    python database/migrations/run_012.py
    # or from project root:
    python -m database.migrations.run_012
"""
import os
import sys
import re
...
def run():
    sql_path = os.path.join(os.path.dirname(__file__), '012_announcement_management.sql')
    ...
    print("🚀 Running Migration 012: Announcement Management System ...")
    ...
    try:
        ...
        conn.commit()
        print(f"\n✅ Migration 012 completed successfully. ({executed}/{len(statements)} statements applied)")

    except mysql.connector.Error as e:
        conn.rollback()
        print(f"\n❌ Migration 012 FAILED at statement {i}: {e}")
